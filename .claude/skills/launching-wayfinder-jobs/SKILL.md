...
- A wish for evidence ("show me how it would have done", "backtest it") on a perp rule means a harnessed `jobs_v1` job with a fetched dataset and a `backtest_job` run, not a freestyle script; a catalog starter that matches the described rule is a fine answer, say so and use it.

## When the ask does not fit a kind

- Perp, spot, spot-perp or prediction-market rules with any trigger ("if the odds cross X, buy Y perp", "buy ETH on Robinhood chain under 2000 and sell above 2500") → a freestyle job. Venues: `hyperliquid` (perps), `hyperliquid_spot` (spot pairs like `HYPE/USDC` on the same account, long-only), `onchain` (spot: any token id on any chain, bought and sold through the swap router, long-only), `polymarket`, `hyperliquid_prediction`.
...
- A described alpha idea on the harnessed universe → a harnessed `jobs_v1` job, so it gets the backtest, holdout and evolution: the Strategy Lab agent when the owner can switch to it, otherwise build it yourself with the five calls in `rules/harnessed.md` (create with `symbols` + `bar_interval`, write the strategy module, `fetch_dataset`, `backtest_job`, `readout`).
