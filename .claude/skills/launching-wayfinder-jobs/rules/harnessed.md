...
1. **Create the job with its data contract in one call:**
   ...
   `symbols` + `bar_interval` seed `execution_spec.data_contract` (completed bars, next-bar-open fills on the venue you name) and sensible `execution_params` (10,000 USD paper capital, the venue's taker costs, a lookback window). `venue` picks where it trades: `hyperliquid` perps (default; 4.5 bps fee, 3.5 bps slippage), `onchain` for spot tokens named by token id (`ethereum-robinhood`, `usd-coin-polygon`, `robinhood_0x…`; intervals `1m|5m|15m|1h|4h|1d`; 30 bps fee, 50 bps slippage; long-only, no brackets, no leverage — the engine refuses shorts and limit orders there in backtest, paper and live alike), or `hyperliquid_spot` for pairs (`HYPE/USDC`; 7/10 bps, long-only). Override any of those through `execution_params`. The onchain defaults are sized for a thin memecoin pool; for a deep stable or blue-chip pool (sUSDe, wstETH, ETH on a major chain) pin realistic costs (`fee_bps` 5–10, `slippage_bps` 5–10) or the round trip eats months of yield and every rotation reads as unsupported. A yield-bearing token is two declarations: its token id as the symbol (the accruing price carries the return) and its yield as a feature (`fetch_yield_features` with `yield_apy:<symbol>`, read in `decide()` through `ctx.view.feature("yield_apy:<symbol>")`) when the yield is the signal; rebasing tokens (stETH, aTokens) keep a flat price and are not modelled — trade the wrapped, accruing form instead. The job is created paused.
