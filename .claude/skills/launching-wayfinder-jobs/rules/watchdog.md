...
Harnessed jobs at `intervene`/`auto` with a canonical dataset are evolution-eligible; a campaign runs about every 48 hours, one per box at a time, oldest-first when many are due; a job whose agent loop is not running never holds the slot, and a denied start is journaled as `evolution_campaign_blocked`. `status.evolution` shows eligibility, the campaign status and `next_due_at`; `status.probation_summary` lists every probation trial (paired days, candidate vs reference PnL, the paired estimate and its bounds). Probation can kill a candidate; it cannot prove one — say so when reading a trial back. The owner can cancel a trial or promote it early (`probation_cancel`, `probation_promote_early`); early promotion lands as a proposal the owner approves, never an apply.
