...
- For real transactions, use MCP `core_run_script(...)` so the safety review hook can show a preview (adapter-based contract interactions run inside a Python script; `core_execute(...)` doesn’t cover them).
