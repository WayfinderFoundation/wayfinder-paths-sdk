...
- **Browse with `onchain_discover_tokens(chain_code, dimension)`** (`trending` | `volume` | `new` | `active`) to see what's actually live — including brand-new launches — with price, liquidity, 24h volume, FDV, pool age, and DEX.
