...
- `summary` — the ONLY place money appears; shape branches on `account_abstraction`:
  - `"unifiedAccount"` → `unified_usdc_total` (USDC owned, including margin holds), `unified_usdc_available` (free to trade or withdraw), `unified_equity` (total + unrealized perp PnL — the account value the HL UI shows; quote this when the user asks how much money they have). Perp margin is held out of spot USDC, so a perp account value of ~0 with no open positions is normal, not "no funds".
