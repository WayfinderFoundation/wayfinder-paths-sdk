...
- `summary` — the ONLY place money appears; shape branches on `account_abstraction`:
  - `"unifiedAccount"` (one USDC ledger backing both perp margin and spot) → mirrors HL's Unified Account card: `unified_usdc_equity` (cash + unrealized PnL — HL's Portfolio Value; quote this when the user asks how much money they have), `unified_usdc_unrealized_pnl`, `unified_usdc_margin_used` (margin committed to open positions), `unified_usdc_margin_available` (free collateral for new positions, spot spends, and withdrawals — per-asset caps still come from `hyperliquid_get_trade_asset`), `unified_usdc_liquidation_floor` (cross liquidates when equity falls to this), `unified_maintenance_ratio` (floor/equity — HL shows it as a percent; liquidation near 1.0), `unified_account_leverage` (position notional / equity). Perp margin is held out of spot USDC, so a perp account value of ~0 with no open positions is normal, not "no funds".
