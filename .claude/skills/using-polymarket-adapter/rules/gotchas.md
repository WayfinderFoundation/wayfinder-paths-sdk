...
- Use the per-action write tools (`polymarket_fund_deposit_wallet`, `polymarket_withdraw_deposit_wallet`, `polymarket_place_market_order`, `polymarket_place_limit_order`, `polymarket_cancel_order`, `polymarket_redeem_positions`). All gated by a confirmation prompt in Claude Code.
...
- Funding the deposit wallet is **explicit** — `polymarket_fund_deposit_wallet(amount=...)`. Order placement does **not** auto-fund.
