...
- **Never offer to deploy a strategy that only looks good in-sample.** That is the curve-fit trap this whole loop exists to avoid. If there is no OOS edge, say so with the numbers — then evolve, don't keep tuning params on a dead signal.
