...
6. **Validate out-of-sample** — `core_jobs(action="experiments", job_id=…, grid={…inline param grid…}, wf_test_bars=240, wf_folds=4)`. It uses a fast bounded rolling train window by default — don't hand-shrink the dataset to make it finish. Judge on **`decay_ratio`** (want near 1) and **`oos_positive_folds`** (want most folds positive). If OOS collapses vs in-sample, it's fit to noise.
