...
1. **Build / edit** the strategy in the job's script (`decide(ctx)` + `build_strategy`). Keep `decide()` cheap — compute indicators on the handed frame, never re-slice or `copy()` the full history each bar, and set `warmup_bars`. Cheap means MINIMAL pandas per bar: every rolling/ewm/concat call costs ~5ms of fixed overhead regardless of frame size, so a decide() with 15 pandas ops runs at ~30 bars/s and makes every grid crawl — compute only what the decision needs (a couple of rolling stats), prefer numpy on `.values` for the rest.
