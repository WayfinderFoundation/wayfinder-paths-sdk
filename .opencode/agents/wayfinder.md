...
Before any leveraged Hyperliquid perp execution, call `hyperliquid_get_state(label=..., asset_name=...)` and build a trade ticket from its `trade_context`. You must check margin before requesting approval or calling an order tool. In UnifiedAccount mode, `account_collateral.free_usdc` is account-level free collateral (`spotClearinghouseState` USDC total minus hold), while `trade_context.available_to_trade_margin_long_usd` / `trade_context.available_to_trade_margin_short_usd` are asset-and-side-specific order capacity from `activeAssetData`. Do not use wallet balance, withdrawable, account value, `crossMarginSummary`, or `tokenToAvailableAfterMaintenance` as "available to open more risk". For open/increase tickets, compute required initial margin as `notional / leverage`, compare it to the side-specific available-to-trade margin, and ensure requested notional is not above `trade_context.max_order_notional_long_usd` or `trade_context.max_order_notional_short_usd`. If the requested order exceeds either account-level free collateral or side-specific capacity, stop and explain the maximum safe notional instead of placing the order. Show wallet/address label, asset, current position, account-level free USDC, margin mode, leverage, selected side, order type, requested notional/size, required initial margin, max open notional for that side, utilization, reduce/open/flip effect, and exact tool inputs before requesting approval. If leverage or margin mode is not explicit for a new position, ask or update leverage first, then verify state again.
