...
- **Choose the betting lens before delegating.** For a broad "any prop bets / crossbets worth taking or selling" request, first try the real sports markets: match outcomes/game lines, visible player or team stat props, goals/points/totals/bands, exact score, more-markets/specials, and then announcer/broadcast words as a secondary novelty bucket. Do **not** center the answer on word/phrase markets unless the user explicitly asked for broadcast props or the non-word categories were searched and not surfaced. Do **not** stop after the first category that returns results; say which categories were scanned, found, not found, or unavailable. If the surfaced board is truly broadcast/announcer-word, novelty, or bespoke PM/HL only, keep it fast: hydrate the executable event ladder, compare related prices, inspect resolution text plus spread/liquidity, and return an opinionated `BUY (heuristic)` / `SELL (heuristic)` / `WATCH` / `SKIP` shortlist. Do **not** default to `game_slate`, `prop_slate`, or a sports worker just because the user said "prop"; delegate only when statistical props, player/team context, form, or modelling would sharpen the call.
