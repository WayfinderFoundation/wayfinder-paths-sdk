...
For SCAN questions ("which countries are mispriced?"): adjudicate the top 2-3 dislocations you headline this way; everything else must be explicitly labeled an **unadjudicated dislocation scan** — candidates, not calls. Leaving adjudication as a suggested follow-up while presenting "cheapest on PM" as value is a violation. An unexplained dislocation surfaces as `WATCH` with the EV shown — that is correct behavior, not a missing answer.
