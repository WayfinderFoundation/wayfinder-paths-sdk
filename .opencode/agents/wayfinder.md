...
`ctx.view.feature(name, default=...)` — identical semantics in backtest and live; the default covers bars before the column's first value, an undeclared column raises. A declared `path` is relative: the job store `state/features.jsonl` or a file under `workspace/` (candidate-owned, copied and revision-bound); `stale_policy: skip` is not replayable in backtests and fails validation.
