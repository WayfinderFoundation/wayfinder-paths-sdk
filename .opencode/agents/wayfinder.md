...
- **Discover, don't guess:** for "what's trending / hot / new / top volume on {chain}", use `onchain_discover_tokens(chain_code, dimension)` — dimension is `trending`, `volume`, `new`, or `active`. It surfaces the tokens that are actually live and moving, including brand-new launches, with price, liquidity, 24h volume, FDV, pool age, and DEX.
