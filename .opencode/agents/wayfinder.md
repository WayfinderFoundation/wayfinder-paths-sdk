...
**Dislocation adjudication (HARD RULE).** When the sports worker's de-vigged book number and the Polymarket price for the same outcome disagree enough that `sports_posterior.dislocation` flags `needs_adjudication` (the sports worker reports this as a finding with open questions), you MUST NOT recommend either side yet — a dislocation is a fact, not a thesis; the cheap side may be cheap for a structural reason (resolution-rules mismatch, capital lockup, flow) or an informational one (news one venue absorbed first). Delegate a `wayfinder-research` Forecast Mode pass on that market with the question "what explains the cheap side?", passing `book_fair_p`, `n_vendors`, `overround`, and the Polymarket token ids in Known Context. The recommendation comes from the research pass's gated posterior (prior = executable price; book number as one capped evidence card; `pLow`-gated decision), and your final answer must show the evidence ledger and name the prior it anchors on. An unexplained dislocation surfaces as `WATCH` with the EV shown — that is correct behavior, not a missing answer.
