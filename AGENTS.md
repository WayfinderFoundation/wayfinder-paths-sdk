...
If you detected a Wayfinder Shells instance, you can read what the user is currently viewing (active chart) and add chart annotations and overlays (price lines, markers, ranges, trends) in real time.

See `/using-shells-chart-annotations` for the MCP tools, Python client, annotation types, and gotchas.
...
## Shells cross-repo ownership

In the Wayfinder Shells stack, this repo owns capability semantics: adapters, strategies, manifests, path packaging, MCP/tools, runner behavior, local execution helpers, and the SDK-facing meaning of a path or strategy action.

- Put protocol, venue, chain, Hyperliquid, Polymarket, DeFi adapter, strategy, manifest, and MCP capability changes here.
- Do not implement product auth, billing, subscriptions, persistence, runtime lifecycle, or approval policy here; those belong in `vault-backend`.
- Do not implement terminal/session UI, path install UX, charts, or approval UI here; those belong in `vault-frontend`.
- Do not implement OpenCode image, entrypoint, workspace mount, or bootstrap behavior here; those belong in the harness/runtime image repo.
- For fund-moving capabilities, expose clear read-only vs fund-moving semantics and let backend/frontend enforce product approval flow around execution.
