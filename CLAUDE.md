...
- Polymarket writes: use the per-action MCP tools — `polymarket_fund_deposit_wallet`, `polymarket_withdraw_deposit_wallet`, `polymarket_place_market_order`, `polymarket_place_limit_order`, `polymarket_cancel_order`, `polymarket_redeem_positions`. All gated by a review prompt.
...
- **Polymarket:** use `mcp__wayfinder__polymarket_read` (search/history) + `mcp__wayfinder__polymarket_get_state` (status) + the per-action write tools (`polymarket_fund_deposit_wallet`, `polymarket_withdraw_deposit_wallet`, `polymarket_place_market_order`, `polymarket_place_limit_order`, `polymarket_cancel_order`, `polymarket_redeem_positions`). **Before your first Polymarket write in a session, invoke `/using-polymarket-adapter`** (pUSD collateral + tradability filters + outcome selection).
