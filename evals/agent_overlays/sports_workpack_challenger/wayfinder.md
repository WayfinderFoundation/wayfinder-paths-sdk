...
For sports or prediction-market HIP-4 search, call `wayfinder_hyperliquid_search_hip4(query="...", limit=15)` so perps/spots are filtered out by construction; only fetch mids for surfaced `#...` assets. Use unfiltered `wayfinder_hyperliquid_search_market` only when the user is asking for asset/perp/spot discovery.
...
- **Choose the betting lens before delegating.** For a broad "any prop bets / crossbets worth taking or selling" request, first try real sports markets: match outcomes/game lines, visible player or team stat props, goals/points/totals/bands, exact score, more-markets/specials, then announcer/broadcast words as a secondary novelty bucket. Secondary means scan after sports props, not skip: if PM/HL search surfaces `more-markets`, specials, exact-score, or announcer/broadcast event groups, hydrate the top liquid/relevant event before a global prop conclusion. Use `wayfinder_hyperliquid_search_hip4`, not unfiltered Hyperliquid search, for this discovery. Do **not** stop after the first category that returns results; say which categories were scanned, found, hydrated, skipped with reason, not found, or unavailable. A broad `NO EDGE` claim is allowed only after surfaced categories are hydrated or explicitly marked skipped; otherwise scope the claim, e.g. "no edge in match outcomes and liquid player props checked." If compaction or a worker handoff says `surfaced_unhydrated`, `blocked`, or `next_steps_remaining`, resume those missing checks or keep the no-edge claim scoped. For live sportsbook `player_props`, use `limit=20` by default and page with `offset=20` only when the first page is still relevant; prefer `prop_type`/`vendors` filters over full-board pulls. Before final BUY/SELL/NO EDGE on a broad sports scan, run a bounded research/context pass for shortlisted or ambiguous markets (current match state, availability/injuries, lineup/news, and resolution facts), but do not run full simulation unless the shortlist needs it. When sportsbook/model context differs from an executable PM/HL price, rank by fair-value delta and explain the gap; lack of a cross-venue arb is not a skip reason. Do **not** default to `game_slate`, `prop_slate`, or a sports worker just because the user said "prop"; delegate only when statistical props, player/team context, form, or modelling would sharpen the call.
...
When delegating, include a `Known Context` block with the `sport`, any `event_id`/`event_ids` (`game_id`, `match_id`, `fight_id`, or `tournament_id` only when specifically known), an existing `run_id` or `model_id` to continue, the bet type (moneyline/spread/over_under/prop), the user's concrete question, `surfacePackRefs`, and every relevant file path written under `.wayfinder_runs/` or `.wayfinder_runs/packs/`. State the requested mode (`SPORTS_SCAN`, `SPORTS_CONTEXT_FEATURES`, `SPORTS_ANALYSIS`, `ANALYZE`, `DECIDE`, `VALIDATE`) and the expected output packs (`analysisPack`, `decisionPack`, `validationReport`, or `resumeRequest`). Game models reject player-prop (`pp_*`) factors — props need a prop-type model.
...
## Eval Variant: WorkPack Challenger

For multi-step work, ask workers to write reusable WorkPacks under
`.wayfinder_runs/packs/` and return `packRefs`. Use generic pack types:
`surfacePack`, `contextPack`, `featurePack`, `analysisPack`, `decisionPack`,
and `validationReport`.

Persisted packs are audit/reuse artifacts, not live truth. Before execution or
an actionable recommendation, rehydrate any expired or execution-sensitive
surface/decision pack. For sports betting, `wayfinder-sports` produces
surface/context/feature/analysis packs, `wayfinder-research` produces
context/evidence packs only, and `wayfinder-quant` produces the decisionPack
plus validationReport.

Do not end with "want me to hydrate/check/search X?" when X is necessary for
the answer. Perform the highest-impact unresolved executable hydration first,
or clearly label the row WATCH/SKIP because the required surface is missing.
