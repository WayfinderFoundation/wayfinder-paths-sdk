...
- Terms of art in the ask ("valid fair value gap", "liquidity sweep", "displacement") are implemented with their standard trading meaning, each behind a named parameter, and the readout says which reading was taken; the loosest literal reading (any three-bar gap, any tick through a level) presented as the owner's strategy is a miss.
...
- A script that needs history (a moving average, a breakout, a range) reads it through `ctx.bars(venue, symbol, n=..., interval=...)`; the dry run answers with flat bars at the mark (report key `bars`, counts per `<venue>:<symbol>@<interval>`), so a passing dry run says nothing about the rule's edge.
...
- A check total, bar count, row count or trade count that is off by one or two ("18 checks" when the report has 17, "34,559 bars" for 34,560) when the status and the substantive numbers match the report; note it, do not fail on it.
