...
There is a live smoke script in `scripts/protocols/aerodrome/aerodrome_smoke.py` that:
...
```bash
poetry run python scripts/protocols/aerodrome/aerodrome_smoke.py --wallet-label main
```

## APY / Deploy helpers

- `scripts/protocols/aerodrome/aerodrome_best_emissions_deploy.py`: ranks Aerodrome v2 gauges by **emissions APR** (onchain) and
  optionally deploys a USDC budget (swap → add-liquidity → stake).
- `scripts/protocols/aerodrome/aerodrome_best_vote_pools.py`: ranks pools by latest-epoch **fees+bribes per veAERO** (via Sugar
  `epochsLatest`) and can optionally `createLock` + `vote`.

## Slipstream (CL) analytics

- `scripts/protocols/aerodrome/slipstream_analyze_range.py`: analyzes a Slipstream CL pool range (ticks, liquidity share),
  estimates volume from Swap logs, and estimates an unstaked fee APR (range-adjusted).
- `scripts/protocols/aerodrome/slipstream_enter_position.py`: simple CL entry helper (swap USDC into both legs, mint a range NFT,
