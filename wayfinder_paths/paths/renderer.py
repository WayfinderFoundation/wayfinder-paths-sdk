...
def _render_bootstrap_script(runtime_manifest: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "DEV_CONFIG_FILENAME = 'config.dev.json'",
            "",
            "",
            ...
            "    if path.name != DEV_CONFIG_FILENAME or path.exists():",
            "        return path",
            "    fallback = path.with_name(DEFAULT_RUNTIME_CONFIG_PATH.name)",
            "    if fallback.exists():",
            "        return fallback",
            "    return path",
            "",
            ...
            "    if not path_value:",
            "        return None",
            "    return _fallback_config_path(Path(path_value).expanduser())",
            "",
            "",
            "def _runtime_env(manifest: dict[str, object]) -> dict[str, str]:",
            "    env = os.environ.copy()",
            "    cfg_env = str(manifest.get('config_path_env') or 'WAYFINDER_CONFIG_PATH')",
            "    resolved_config = _resolve_config_path(env.get(cfg_env))",
            "    if resolved_config is not None:",
            "        env[cfg_env] = str(resolved_config)",
