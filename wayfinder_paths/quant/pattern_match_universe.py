...
def load_calibration_bundle() -> dict[str, Any]:
    """Frozen research thresholds and costs; no fitting or network access."""
    return json.loads(
        files("wayfinder_paths.quant")
        .joinpath("pattern_match_universe_calibration.json")
        .read_text()
    )
    ...
    *,
    ...
    for item in resolved:
        if str(item.get("symbol")) != symbol:
            continue
        try:
            item_direction = int(item["direction"])
...
def _score_window(
    ...
    overlay_series: dict[str, Any] = {}
    if config.display_matches:
        forward = closes[
            selected_ends[:, None] + np.arange(1, max(config.forecast_horizons) + 1)
        ]
        forward = np.column_stack(
            (np.zeros(len(ranking)), forward / closes[selected_ends, None] - 1)
        )
        overlay_series = {
            "id": "same_market",
            "label": "Same-market analogues",
            "source": "hyperliquid",
            "sample_count": len(ranking),
            "median_bps": (np.quantile(forward, 0.5, axis=0) * 10_000).tolist(),
            "q25_bps": (np.quantile(forward, 0.25, axis=0) * 10_000).tolist(),
            "q75_bps": (np.quantile(forward, 0.75, axis=0) * 10_000).tolist(),
            "hit_rate_up": (forward > 0).mean(axis=0).tolist(),
            "analogues": [
                {
                    "start_ms": int(timestamps_ns[end - window + 1] // 1_000_000),
                    "end_ms": int(timestamps_ns[end] // 1_000_000),
                    "similarity_score": float(selected_similarity[index]),
                    "forward_path_bps": (forward[index] * 10_000).tolist(),
                }
                for index, end in enumerate(selected_ends[: config.display_matches])
            ],
        }
    return WindowForecast(
        ...
        probabilities=dict(
            zip(config.forecast_horizons, probabilities.tolist(), strict=True)
        ),
        range_q25=float(np.quantile(total_ranges, 0.25)),
        range_q75=float(np.quantile(total_ranges, 0.75)),
...
def _select_non_overlapping(
    ...
    *,
    ...
    chosen: list[int] = []
    # Starts are integer bar offsets. Mark forbidden offsets instead of doing
    # an O(pool_size) Python scan for every candidate (same greedy ordering).
    blocked = (
        np.zeros(int(starts.max()) + separation + 1, dtype=bool)
        if len(starts)
    ...
    for index in order:
        ...
        blocked[max(0, start - separation + 1) : start + separation] = True
        if len(chosen) == top:
            break
