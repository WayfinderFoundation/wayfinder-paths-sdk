...
def test_declared_feeds_skip_undeclared_features_and_journal_failures(tmp_path) -> None:
    ...
    assert refresh_venue_feeds(job_id, store=store) == {
        "funding": None,
        "open_interest": None,
    }
...
def test_wake_refresh_runs_the_declared_feeds(tmp_path, monkeypatch) -> None:
    ...
    seen: list[str] = []
    monkeypatch.setattr(
        "wayfinder_paths.jobs.derived_features.refresh_venue_feeds",
    ...
    assert seen == [job_id]
    assert result["venue_feeds"] == {
        "funding": None,
        "open_interest": {"appended": 2},
    }
