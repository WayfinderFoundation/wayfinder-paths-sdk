...
async def test_strategy_not_found():
    with patch(
        ...
        out = await core_run_strategy(strategy="nope", action="status")
    assert out["ok"] is False
...
async def test_analyze_not_supported():
    ...
    with _patch_load(strategy_class=cls), _patch_config(), _patch_signing():
        out = await core_run_strategy(strategy="my_strat", action="analyze")
    assert out["ok"] is False
...
async def test_snapshot_not_supported():
    ...
    with _patch_load(strategy_class=cls), _patch_config(), _patch_signing():
        out = await core_run_strategy(strategy="my_strat", action="snapshot")
    assert out["ok"] is False
    assert out["error"]["code"] == "not_supported"
...
async def test_quote_not_supported():
    ...
    with _patch_load(strategy_class=cls), _patch_config(), _patch_signing():
        out = await core_run_strategy(strategy="my_strat", action="quote")
    assert out["ok"] is False
    assert out["error"]["code"] == "not_supported"
...
async def test_update_success():
    with _patch_load(), _patch_config(), _patch_signing():
        out = await core_run_strategy(strategy="my_strat", action="update")
    assert out["ok"] is True
...
async def test_withdraw_success():
    with _patch_load(), _patch_config(), _patch_signing():
        out = await core_run_strategy(strategy="my_strat", action="withdraw")
    assert out["ok"] is True
...
async def test_exit_success():
    with _patch_load(), _patch_config(), _patch_signing():
        out = await core_run_strategy(strategy="my_strat", action="exit")
    assert out["ok"] is True
...
async def test_setup_called_before_action():
    ...
    with _patch_load(strategy_class=cls), _patch_config(), _patch_signing():
        await core_run_strategy(strategy="my_strat", action="status")
